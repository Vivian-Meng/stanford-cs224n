...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens_padded, (last_hidden, last_cell) = self.encoder(X_packed) ### (src_len,b,h*2), (2, b, h), (2,b,h)

        enc_hiddens = pad_packed_sequence(enc_hiddens_padded)[0].permute(1,0,2) ### (b, src_len, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) #(b, src, h)


        Y = self.model_embeddings.target(target_padded) # (tg_len-1, b, e)

        for i in range(Y.size(0)):
            ybar_t = torch.cat((Y[i],o_prev),1) #(b, e+h)
